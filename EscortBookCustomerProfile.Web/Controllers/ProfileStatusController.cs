...
public class ProfileStatusController : ControllerBase
{
    ...
    {
        var profileStatus = await _profileStatusRepository.GetAsync(ps => ps.ID == id);

        if (profileStatus is null) return new NotFoundObjectResult(new
        {
            Message = "Profile status not found"
        });

        var category = await _profileStatusCategoryRepository
            .GetAsync(psc => psc.ID == profileStatus.ProfileStatusCategoryID);

        if (category is null) return new NotFoundObjectResult(new
        {
    ...
    public async Task<IActionResult> UpdateByExternalAsync(
        [FromRoute] string id,
        [FromBody] UpdateProfileStatus profile,
        [FromHeader(Name = "user-email")] string userEmail,
        [FromHeader(Name = "user-type")] string userType
    )
    {
        var category = await _profileStatusCategoryRepository
            .GetAsync(psc => psc.ID == profile.ProfileStatusCategoryID);
    ...
    )
    {
        ...
        var profileStatus = await _profileStatusRepository.GetAsync(ps => ps.CustomerID == userId);
